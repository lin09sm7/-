...
function App() {
  ...
  useEffect(() => {
    axios.get(`${API}/cities`).then((res) => {
      setCities(res.data.map((c: { id: number; name: string }) => ({ value: String(c.id), label: c.name })));
    });
    axios.get(`${API}/departments`).then((res) => {
      setDepartments(res.data.map((d: { id: number; name: string }) => ({ value: String(d.id), label: d.name })));
    });
  }, []);
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/patients`, { params: { medical_no: medicalNo } }).then((res) => {
      ...
      // district needs city to load first, set after a tick
      if (p.city_id) {
        axios.get(`${API}/cities/${p.city_id}/districts`).then((dRes) => {
          setDistricts(dRes.data.map((d: { id: number; name: string }) => ({ value: String(d.id), label: d.name })));
          setDistrictId(p.district_id ? String(p.district_id) : "");
        });
      }
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/cities/${cityId}/districts`).then((res) => {
      setDistricts(res.data.map((d: { id: number; name: string }) => ({ value: String(d.id), label: d.name })));
    });
  }, [cityId]);
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/departments/${departmentId}/doctors`).then((res) => {
      setDoctors(res.data.map((d: { id: number; name: string }) => ({ value: String(d.id), label: d.name })));
    });
  }, [departmentId]);
  ...
  const handleConfirm = async () => {
    if (!medicalNo || !patientName || !regDate || !departmentId || !doctorId) {
      alert("請填寫必要欄位：病歷號碼、姓名、掛號日期、科別、醫師");
      return;
    }
    ...
    try {
      ...
      if (existing.data) {
        ...
        // Create new patient
        const patientRes = await axios.post(`${API}/patients`, patientData);
    ...
    <div className="flex h-screen items-center justify-center p-4">
      ...
      >
        ...
        >
          ...
          <div className="flex items-end gap-4">
            <FormSelect label="縣市" width="140px" value={cityId} onChange={setCityId} options={cities} />
            <FormSelect label="區域" width="140px" value={districtId} onChange={setDistrictId} options={districts} />
        ...
        >
          ...
          <div className="flex items-end gap-4">
            ...
            <FormSelect label="科別" width="180px" value={departmentId} onChange={setDepartmentId} options={departments} />
            <FormSelect label="醫師" width="180px" value={doctorId} onChange={setDoctorId} options={doctors} />
          </div>
